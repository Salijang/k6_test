...
| 구분 | 작성 시점 | 포함 내용 |
| --- | --- | --- |
| 실행 전 작성 | capacity 테스트 실행 전 | 환경, 1차 한계, 보완 테스트 설계, 판단 기준, 실행 방법 |
| 실행 후 작성 | capacity 테스트 실행 후 | 단계별 결과, 해석, 수용 구간 판단, 최종 결론, 개선 과제 |

작성 원칙:
- `[실행 전]` 표시가 있는 섹션은 capacity 테스트 실행 전에 먼저 작성한다.
- `[실행 후]` 표시가 있는 섹션은 실제 `k6` step-load 실행 결과와 대시보드 확인 후 확정한다.
...
### 3.1.1 본 보완 보고서에서 명시할 관찰 항목

- 본 보완 테스트는 `최대 동접 수`만 보는 것이 아니라 `avg/p95/max 응답시간`, `HTTP Requests / s`, `Iterations / s`, `Check Pass Rate`, `2xx/4xx/5xx 분포`, `Dropped Iterations` 를 함께 본다.
- 각 단계에서 `200` 유지 여부, `5xx` 발생 시점, `check 실패` 증가 시점, 응답시간 급증 시점을 함께 확인한다.
- 따라서 capacity 판단은 `버틴 최대 VU 수` 하나로만 내리지 않고, `안정 운영 가능 구간`, `성능 저하 시작 구간`, `명확한 한계 구간`으로 나눠 기술한다.

### 3.1.2 스크립트 동작 설명

- 이 스크립트는 상품 목록 조회 API 하나에 집중해, 동시 사용자 수를 단계적으로 올리면서 각 구간의 응답시간과 오류 증가 시점을 관찰한다.
- 각 단계는 `램프업 -> 유지` 구조로 구성되어 있어, 특정 VU 구간에 진입한 직후의 변화와 일정 시간 유지했을 때의 안정성을 함께 볼 수 있다.
- 응답 상태는 `2xx`, `4xx`, `5xx` 로 나눠 카운트하고, `200` 응답과 배열 형태 응답 여부를 `check` 로 검증한다.
- 조회 API를 대상으로 삼은 이유는 데이터 변형이 없어 반복 실행의 재현성이 높고, step-load 에 따른 수용 한계를 가장 먼저 비교하기 좋기 때문이다.

...
```bash
./k6aws read --rate 5 --duration 1m --wait
```
...
| 방식 | 실행 위치 | 필요 조건 | 용도 |
| --- | --- | --- | --- |
| `./k6aws ...` 또는 `tests/performance/run-aws-k6.sh ...` | EC2 k6 runner | 로컬 AWS CLI 권한 | 공식 AWS 부하테스트 |
...
- `./k6aws` / `run-aws-k6.sh`: 로컬에서 실행해도 내부적으로 SSM `send-command`를 사용하므로 실제 k6는 EC2 runner에서 실행된다.
...
팀 공용 AWS 부하테스트 표준은 repo 루트의 `./k6aws`다.
긴 원본 경로가 필요하면 `tests/performance/run-aws-k6.sh`를 직접 실행해도 된다.
...
`./k6aws`와 `tests/performance/run-aws-k6.sh`는 내부에서 `aws ssm send-command`를 사용한다.
...
`./k6aws`는 테스트 실행 전에 runner 상태를 확인한다.
...
`./k6aws`는 repo 루트에서 쓰는 짧은 실행 명령이다.
내부적으로는 `tests/performance/run-aws-k6.sh`를 호출한다.
아래처럼 긴 경로로 실행해도 같은 동작이다.
